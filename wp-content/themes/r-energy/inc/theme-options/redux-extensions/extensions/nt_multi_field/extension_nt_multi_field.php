        ...
        private function is_minimum_version () {
            ...
            if ($this->min_redux_version != '') {
                if (version_compare($redux_ver, $this->min_redux_version) < 0) {
                    $msg = '<strong>' . esc_html__( 'The', 'r-energy') . ' ' .  $this->ext_name . ' ' .  esc_html__('extension requires', 'r-energy') . ' Redux Framework ' . esc_html__('version', 'r-energy') . ' ' . $this->min_redux_version . ' ' .  esc_html__('or higher.','r-energy' ) . '</strong>&nbsp;&nbsp;' . esc_html__( 'You are currently running', 'r-energy') . ' Redux Framework ' . esc_html__('version','r-energy' ) . ' ' . $redux_ver . '.<br/><br/>' . esc_html__('This field will not render in your option panel, and features of this extension will not be available until the latest version of','r-energy' ) . ' Redux Framework ' . esc_html__('has been installed.','r-energy' );
                    ...
                    if (method_exists('Redux_Admin_Notices', 'set_notice')) {
                        ...
                        echo     '<p>';
                        echo r_energy_sanitize_data($msg);
                        echo     '</p>';
                        echo '</div>';
                    }
