...
//
//     //true or false
//     console.log(popup_instance.get_conversion());
//   });
// });
...
window.MasterPopups = (function ($, window, document, undefined) {
  ...
  MasterPopups.prototype = {
      ...
      _.$popup.find('.mpp-element-field_submit').on('click', function (event) {
        ...
        $processing_form.fadeIn(200, function (e) {
          ...
          //Validate form
            ...
            _.process_form(event, $processing_form);
          }
        });
      });
    ...
    open: function (event) {
      ...
      setTimeout(function () {
        ...
        setTimeout(function () {
          ...
          if (_.get_conversion() && app.show_popup_on_conversion(_.options)) {
    ...
    get_form_data: function (event) {
      ...
      if( event ){
        device = $(event.target).closest('.mpp-element').data('device');
      }
    ...
    process_form: function (event, $processing_form) {
      var _ = this;
      var $content = $processing_form.find('.mpp-processing-form-content');
      var data = _.get_form_data(event);
              ...
              if (response.actions.redirect) {
                setTimeout(function () {
                  if (_.$body.hasClass('wp-admin')) {
                    ...
                    app.redirect(response.actions.redirect_to, response.actions.redirect_target);
                  }
                }, 1500);
              }
              if (response.actions.advanced_redirection) {
                setTimeout(function () {
                  if (_.$body.hasClass('wp-admin')) {
                    ...
                    app.redirect(response.actions.advanced_redirection, response.actions.redirect_target);
                  }
                }, 1500);
              }
              _.update_submits();
    ...
    validate_form: function (event, $form) {
      ...
      $form.find('.mpp-form-element').each(function (index, el) {
        ...
        message = validation_message || MPP_PUBLIC_JS.strings.validation.general;
  ...
  $(function (event) {
    ...
    $(document).on('click', '.mpp-box', function (e) {
      var popup_id = $(this).closest('.mpp-box').data('popup-id');
