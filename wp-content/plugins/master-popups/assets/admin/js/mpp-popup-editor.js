window.MppPopupEditor = (function (window, document, $) {
  ...
  app.init_countdowns = function () {
    app.$canvas.find('.ampp-element').each(function (index, element) {
  ...
  app.add_countdown_message = function () {
    if( $(this).val() == 'on' ){
      app.$canvas.find('#mc-types').find('.xbox-add-group-item[data-item-type="text-html"]').trigger('click');
      setTimeout(function () {
  ...
  app.remove_elements = function (items) {
    if (items.length) {
      xbox.remove_group_items(items);
      // $(items).each(function(i, $element){
      //     //app.remove_element($element);
      //
      // });
    }
  };
