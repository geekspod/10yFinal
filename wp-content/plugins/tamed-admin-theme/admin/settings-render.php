...
<div class="wrap tamed-wrap layout-<?php echo $option_theme; ?>">
  ...
  <form method="post" action="options.php">
        ...
        function menu_print() {
          echo '<ul id="tamed-menu-order">';
            ...
            foreach( $items as $item ){
              ...
              $item_name = preg_replace('/[0-9]+/', '', $item_name);
              $item_page = $item[2];
              ( (empty($item[0])) ? $item_slug = 'separator' : $item_slug = $item[5] );
              $item_value = get_option('tamed_menu_name_' . $item_slug, $item_name);
              ?>

              <?php if( $item[2] == 'options-general.php' ): ?>

                <li data-page="<?php echo $item_page; ?>" data-name="" draggable="true">
                  <p><?php echo (($item_value) ? $item_value : $item_name); ?></p>
                  <input type="text" name="tamed_menu_name_<?php echo $item_slug; ?>" value="<?php echo (($item_value) ? $item_value : $item_name); ?>">
                  <a class="button menu-item-save">submit</a>
                  <span class="button button-small menu-item-edit">edit</span>
                </li>

              <?php elseif( $item[2] == 'separator1' || $item[2] == 'separator2' || $item[2] == 'separator-last' ): ?>
              ...
              <?php elseif( $item_page == 'edit-comments.php' ): ?>

                <li data-page="<?php echo $item_page; ?>" data-name="" draggable="true">
                  <p><?php echo (($item_value) ? $item_value : 'Comments'); ?></p>
                  <input type="text" name="tamed_menu_name_<?php echo $item_slug; ?>" value="<?php echo (($item_value) ? $item_value : $item_name); ?>">
                  <a class="button menu-item-save">submit</a>
                  <span class="button button-small menu-item-edit">edit</span>
                  <span class="button button-small menu-item-remove">remove</span>
    ...
    <!-- Option: frontend admin bar styling -->
    <div id="tamed-custom-footer" class="tamed-box">
      <div class="tamed-box-header">
        <h2>Admin bar frontend</h2>
      </div>

      <div class="tamed-box-body">
        <p><strong>If checked, this will apply styling to the admin bar on the frontend.</strong></p>

        <?php $option_bar_frontend = get_option('tamed_bar_frontend'); ?>
        <label>
          <input type="checkbox" id="tamed_bar_frontend_enabled" name="tamed_bar_frontend" value="enabled" <?php checked('enabled', get_option('tamed_bar_frontend'), true); ?>>
          Style the frontend admin bar
        </label>
      </div>
    </div>
