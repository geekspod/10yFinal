...
class Forget_password extends CI_Controller 
{
    ...
    {
        parent::__construct();
        $this->load->model('manager/Model_forget_password');
    }
...
//echo "<pre>";print_r( $data['setting']);
        if(isset($_POST['form1'])) {
            ...
            if($valid == 1) {
                ...
                // Send Email
                $msg = '<p>To reset your password, please <a href="'.base_url().'manager/reset-password/index/'.$_POST['email'].'/'.$token.'">click here</a> and enter a new password';
                 $config = array();
				...
				redirect(base_url().'manager/forget_password');
            } else {
                $this->session->set_flashdata('error',$error);
				redirect(base_url().'manager/forget_password');
            }            
        } else {
            $this->load->view('manager/view_forget_password',$data);    
        }
